...
const get_completion = async (prompt, model = 'gpt-3.5-turbo') => {
        ...
        // 调用OpenAI API，创建一个聊天完成请求
        const response = await client.chat.completions.create({
            model, // 使用指定的模型
            messages, // 使用创建的消息数组
            temperature: 0, // 设置生成结果的随机性为0，即最可能的结果
        });
    ...
    //     所以它看起来还不错，但如果你看看底座，刀片锁定到位的部分看起来不如几年前的版本那么好，但我打算非常温柔地使用它
    //     （例如，我先在搅拌机中粉碎非常硬的物品，如豆子、冰、米饭等。
    //     然后在搅拌机中将它们粉碎成我想要的份量，然后切换到搅拌刀片以获得更细的面粉，并在制作冰沙时先使用十字切割刀片，然后如果我需要它们更细/更少的浆状，则使用平刀片）。
    //     制作冰沙的特别提示：将水果和蔬菜切碎并冷冻（如果使用菠菜，请稍微炖一下，然后冷冻直至可以使用；如果制作冰糕，请使用小型或中型食品加工机），这样就可以避免在制作冰沙时添加太多冰块。
    //     大约一年后，电机发出奇怪的声音。
    ...
    // 评论以三个反引号分隔。
    // 将你的回复格式化为 JSON 对象，其中"商品"和"品牌"是作为键。
    // 如果信息不存在，请使用"未知"作为值。
    // 让你的回复尽可能简短。
